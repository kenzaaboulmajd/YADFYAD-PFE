const BASE_URL = "http://localhost/YADFYAD%20PFE"
...
followButtons.forEach(followButton => followButton.addEventListener("click", async () => {
    ...
    if (follow.is_following) { followButton.classList.add("following"); followStatusElement.innerHTML = "Suivi(e)" } else { followButton.classList.remove("following"); followStatusElement.innerHTML = "Suivre" }
}));
...
likeButtons.forEach(likeButton => likeButton.addEventListener("click", async () => {
    ...
    if (follow.is_liking) { likeButton.classList.add("liking") } else { likeButton.classList.remove("liking"); }
    if (follow.count) { likeButton.nextElementSibling.innerHTML = follow.count } else { likeButton.nextElementSibling.innerHTML = 0 };
}));